...
public class LoadWebInfor {
	...
	public ArrayList<String> BrowserList = new ArrayList<String>();// 存放待測試之Borwser
	XSSFWorkbook workBook;
	XSSFSheet Sheet;
	String DriverPath, URL;
	...
	public LoadWebInfor() {
		try {
			...
			try {
				do {
					BrowserList.add(Sheet.getRow(i).getCell(0).toString());// 存放待測試之Borwser清單
					i++;
			...
			for (int j = 0; j < BrowserList.size(); j++) {
				System.out.print(BrowserList.get(j).toString() + "/");
